...
class KSImportTaskTest {
    ...
    void noCommandParam() {
        ...
        assertTrue(command.contains('"""-importcert""","""-noprompt""","""-storepass"""'))
    }
    ...
    void withVerboseParam() {
        ...
        var script = project.tasks.ksImport.executeCommand()
        var command = PSCommandDecoder.decode(script)
        assertTrue(command.contains('"""-importcert""","""-noprompt""","""-v""","""-storepass"""'))
    }
    ...
    void withAliasParam() {
        ...
        var script = project.tasks.ksImport.executeCommand()
        var command = PSCommandDecoder.decode(script)
        assertTrue(command.contains('"""-importcert""","""-noprompt""","""-alias""","""alias""","""-storepass"""'))
    }
    ...
    void withStoreTypeParam() {
        ...
        var script = project.tasks.ksImport.executeCommand()
        var command = PSCommandDecoder.decode(script)
        assertTrue(command.contains('"""-importcert""","""-noprompt""","""-storetype""","""storeType""","""-storepass"""'))
    }
